...
public class CompositeIntegration
		implements MovementsService, AccountService, CustomerService, ProductService, CreditService {
	...
	public CompositeIntegration(RestTemplate restTemplate, ObjectMapper mapper,
		...
		this.urlMovementsService = String.format("http://%s:%s/%s", movementsServiceHost, movementsServicePort,
				"movements");
		this.urlCustomerService = String.format("http://%s:%s/%s", customerServiceHost, customerServicePort,
				"customer");
	...
	public Flux<MovementsReportDTO> getAllMovements() {
		...
		try {
			List<MovementsReportDTO> list = restTemplate
					.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<MovementsReportDTO>>() {
					}).getBody();
	...
	public Flux<MovementsReportDTO> getAllMovementsByNumberAccount(String accountNumber) {
		...
		try {
			...
			logger.warn("Got exception while make CompositeIntegration::getAllMovementsByNumberAccount:  "
					+ ex.getMessage());
			throw handleHttpClientException(ex);
		}
	}
	...
	public Flux<AccountDTO> getAllAccountByCustomer(String customerId) {
		...
		try {
			List<AccountDTO> list = restTemplate
					.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<AccountDTO>>() {
					}).getBody();
	...
	public Flux<CustomerDto> getAllCustomers() {
		...
		try {
			List<CustomerDto> list = restTemplate
					.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<CustomerDto>>() {
					}).getBody();
	...
	public Flux<ProductDTO> getAllProducts() {
		...
		try {
			List<ProductDTO> list = restTemplate
					.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<ProductDTO>>() {
					}).getBody();
	...
	public Mono<CreditDTO> getByCreditNumber(String creditNumber) {
		...
		try {
			CreditDTO dto = restTemplate
					.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<CreditDTO>() {
					}).getBody();
	...
	public Flux<CreditDTO> getAllCreditByCustomer(String customerId) {
		...
		try {
			List<CreditDTO> list = restTemplate
					.exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<CreditDTO>>() {
					}).getBody();
